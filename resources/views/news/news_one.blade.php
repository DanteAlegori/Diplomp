...
@section('content')
    <div class="container py-5">
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <div class="card mb-5">
                    <img src="{{ asset('../public/img/' . $News->img) }}" class="card-img-top" alt="{{ $News->name }}">
                    <div class="card-body">
                        <h1 class="card-title display-5 fw-bold text-body-emphasis mb-4">{{ $News->name }}</h1>
                        <p id="comment" class="card-text mb-5">{!! html_entity_decode($News->content) !!}</p>
            ...
            @if ($comment->approved)
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <p id="content_{{ $comment->id }}" class="card-text mb-5">{!! html_entity_decode($comment->content) !!}</p>
    ...
    <script>
        ...
        @foreach ($comments as $comment)
            @if ($comment->approved)
                tinymce.get('content_{{ $comment->id }}').setContent(
                    '{{ ($comment->content) }}');
            @endif
        @endforeach
    </script>
